...
//								 unsigned int save_as_fixed, u//nsigned int decimal_point);
...
//fann_type fann_activation_derived(unsigned int activation_function, fann_type steepness, fann_type value, fann_type sum);
void fann_activation_derived(unsigned int activation_function, fann_type steepness, fann_type value, fann_type sum, fann_type* ret);
...
#define fann_safe_free(x) {if(x) { kfree(x); x = NULL; }}
#define fann_clip(x, lo, hi) (((x) < (lo)) ? (lo) : (((x) > (hi)) ? (hi) : (x)))
#define fann_exp2(x, ret) (mexp(0.69314718055994530942*(x), &ret))
