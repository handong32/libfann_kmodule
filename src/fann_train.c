...
/* INTERNAL FUNCTION
  Calculates the derived of a value, given an activation function
   and a steepness
*/
void fann_activation_derived(unsigned int activation_function,
			     fann_type steepness, fann_type value, fann_type sum, fann_type* ret)
{
...
/* INTERNAL FUNCTION
  Calculates the activation of a value, given an activation function
   and a steepness
*/
void fann_activation(struct fann * ann, unsigned int activation_function, fann_type steepness,
		     fann_type value, fann_type* ret)
{
	value = fann_mult(steepness, value);
	//fann_activation_switch(activation_function, value, value);
	fann_activation_switch(activation_function, value, ret);
        //*ret = value;
	return;
}
...
*/
void fann_update_MSE(struct fann *ann, struct fann_neuron* neuron, fann_type neuron_diff, fann_type* ret)
{
	float neuron_diff2;
											...
											fann_type * desired_output)
{
	...
	for(output_it = output_begin; output_it != output_end; output_it++)
	{
		...
		fann_update_MSE(ann, output_neuron, neuron_diff, &neuron_diff);
 ...
 */
FANN_EXTERNAL void FANN_API fann_get_MSE(struct fann *ann, float* ret)
{
	if(ann->num_MSE)
	{
	        *ret = ann->MSE_value / (float) ann->num_MSE;
	}
	else
	{
	        *ret = 0;
...
void fann_compute_MSE(struct fann *ann, fann_type * desired_output)
{
	...
	{
	    ann->train_errors = (fann_type *) kcalloc(ann->total_neurons, sizeof(fann_type), GFP_KERNEL);
		if(ann->train_errors == NULL)
		{
	...
	for(; last_layer_begin != last_layer_end; last_layer_begin++)
	{
		...
		fann_update_MSE(ann, last_layer_begin, neuron_diff, &neuron_diff);
		...
		{						/* TODO make switch when more functions */
			...
			{
			    mlog((1.0 + neuron_diff) / (1.0 - neuron_diff), &tmp);
			    neuron_diff = (fann_type)(tmp);
			    //neuron_diff = (fann_type) log((1.0 + neuron_diff) / (1.0 - neuron_diff));
			}
		}

		fann_activation_derived(last_layer_begin->activation_function, last_layer_begin->activation_steepness, neuron_value, last_layer_begin->sum, &tmp2);
		*error_it = tmp2 * neuron_diff;
...
void fann_backpropagate_MSE(struct fann *ann)
{
	...
	for(layer_it = last_layer - 1; layer_it > second_layer; --layer_it)
	{
		...
		{
		    fann_activation_derived(neuron_it->activation_function, neuron_it->activation_steepness, neuron_it->value, neuron_it->sum, &tmp2);
		    *error_prev_layer *= tmp2;
...
void fann_update_weights(struct fann *ann)
{
	...
	if(ann->prev_weights_deltas == NULL)
	{
		ann->prev_weights_deltas =
		    (fann_type *) kcalloc(ann->total_connections_allocated, sizeof(fann_type), GFP_KERNEL);
		if(ann->prev_weights_deltas == NULL)
		{
			fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
			return;
...
void fann_clear_train_arrays(struct fann *ann)
{
	...
	{
		ann->prev_steps = (fann_type *) kmalloc(ann->total_connections_allocated * sizeof(fann_type), GFP_KERNEL);
		if(ann->prev_steps == NULL)
		{
			fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
			return;
...
void fann_update_weights_sarprop(struct fann *ann, unsigned int epoch, unsigned int first_weight, unsigned int past_end)
{
	...
	for(; i != past_end; i++)
	{
		...
		fann_exp2(-T * epoch + weight_decay_shift, tmp);
		slope = -train_slopes[i] - weights[i] * (fann_type)(tmp);
...
fann_get_activation_steepness(struct fann *ann, int layer, int neuron, fann_type* ret)
{
	struct fann_neuron* neuron_it = fann_get_neuron(ann, layer, neuron);
	if(neuron_it == NULL)
	{
	...
	else
	{
	    ...
	    *ret = neuron_it->activation_steepness;
	    return;
	}
}
...
FANN_SET(fann_type, bit_fail_limit)
