 ...
 */
void fann_train_epoch_quickprop(struct fann *ann, struct fann_train_data *data, float* ret)
{
	unsigned int i;
 ...
 * Internal train function 
 */
void fann_train_epoch_irpropm(struct fann *ann, struct fann_train_data *data, float* ret)
{
 ...
 * Internal train function 
 */
void fann_train_epoch_sarprop(struct fann *ann, struct fann_train_data *data, float* ret)
{
	unsigned int i;
 ...
 * Internal train function 
 */
void fann_train_epoch_batch(struct fann *ann, struct fann_train_data *data, float* ret)
{
	unsigned int i;
 ...
 * Internal train function 
 */
void fann_train_epoch_incremental(struct fann *ann, struct fann_train_data *data, float* ret)
{
	unsigned int i;
 ...
 */
FANN_EXTERNAL void FANN_API fann_train_epoch(struct fann *ann, struct fann_train_data *data, float* ret)
{
	if(fann_check_input_output_sizes(ann, data) == -1)
	{
	    *ret = 0;
	...
	case FANN_TRAIN_QUICKPROP:
	    ...
	    fann_train_epoch_quickprop(ann, data, ret);
	    return;
	case FANN_TRAIN_RPROP:
	    //return fann_train_epoch_irpropm(ann, data);
	    fann_train_epoch_irpropm(ann, data, ret);
	    return;
	case FANN_TRAIN_SARPROP:
	    //return fann_train_epoch_sarprop(ann, data);
	    fann_train_epoch_sarprop(ann, data, ret);
	    return;
	case FANN_TRAIN_BATCH:
	    //return fann_train_epoch_batch(ann, data);
	    fann_train_epoch_batch(ann, data, ret);
	    return;
	case FANN_TRAIN_INCREMENTAL:
	    //return fann_train_epoch_incremental(ann, data);
	    fann_train_epoch_incremental(ann, data, ret);
	    return;
	}
	*ret = 0;
	return;
											   ...
											   float desired_error)
{
	...
	for(i = 1; i <= max_epochs; i++)
	{
	    ...
	    fann_train_epoch(ann, data, &error);
			...
			if(ann->callback == NULL)
			{
			    ...
			    printk("Epochs     %8d. Current error: %s, Bit fail %d.\n", i, tbuf,
				   ann->num_bit_fail);
									  ...
									  desired_error, i)) == -1)
			{
			    printk("you can break the training by returning -1 \n");
				/*
																	 ...
																	 struct fann_train_data *data2)
{
	...
	struct fann_train_data *dest =
	    (struct fann_train_data *) kmalloc(sizeof(struct fann_train_data), GFP_KERNEL);
	...
	dest->input = (fann_type **) kcalloc(dest->num_data, sizeof(fann_type *), GFP_KERNEL);
	if(dest->input == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
	...
	dest->output = (fann_type **) kcalloc(dest->num_data, sizeof(fann_type *), GFP_KERNEL);
	if(dest->output == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(dest);
	...
	data_input = (fann_type *) kcalloc(dest->num_input * dest->num_data, sizeof(fann_type), GFP_KERNEL);
	if(data_input == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(dest);
	...
	data_output = (fann_type *) kcalloc(dest->num_output * dest->num_data, sizeof(fann_type), GFP_KERNEL);
	if(data_output == NULL)
	{
		fann_error((struct fann_error*)data1, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(dest);
...
FANN_EXTERNAL struct fann_train_data * FANN_API fann_create_train(unsigned int num_data, unsigned int num_input, unsigned int num_output)
{
	...
	data->input = (fann_type **) kcalloc(num_data, sizeof(fann_type *), GFP_KERNEL);
	if(data->input == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
	...
	data->output = (fann_type **) kcalloc(num_data, sizeof(fann_type *), GFP_KERNEL);
	if(data->output == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
	...
	data_input = (fann_type *) kcalloc(num_input * num_data, sizeof(fann_type), GFP_KERNEL);
	if(data_input == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
	...
	data_output = (fann_type *) kcalloc(num_output * num_data, sizeof(fann_type), GFP_KERNEL);
	if(data_output == NULL)
	{
		fann_error(NULL, FANN_E_CANT_ALLOCATE_MEM);
		fann_destroy_train(data);
		...
		for( cur_neuron = 0; cur_neuron < ann->num_##where##put; cur_neuron++ )	\
			...
			for( cur_neuron = 0; cur_neuron < ann->num_##where##put; cur_neuron++ )	\
			    ...
			    *						\
				...
				for( cur_neuron = 0; cur_neuron < ann->num_##where##put; cur_neuron++ )	\
				{					\
				    msqrt(ann->scale_deviation_##where[ cur_neuron ] / (float)data->num_data, &tmp); \
				    ann->scale_deviation_##where[ cur_neuron ] = (float)(tmp); \
				}					\
