										...
										float desired_error)
{
	...
	{
		printk("Max neurons %3d. Desired error: %.6f\n", max_neurons, desired_error);
	}
	...
	for(i = 1; i <= max_neurons; i++)
	{
			...
			|| i == max_neurons || i == 1 || desired_error_reached == 0))
		{
			if(ann->callback == NULL)
			{
				...
				{
					printk(". candidate steepness %.2f. function %s", 
	...
	if(neurons_between_reports && ann->callback == NULL)
	{
	    fann_get_MSE(ann, &tmp);
	    printk("Train outputs    Current error: %.6f. Epochs %6d\n", tmp,
	    		   total_epochs);
	}
...
int fann_train_outputs(struct fann *ann, struct fann_train_data *data, float desired_error)
{
	...
	fann_train_outputs_epoch(ann, data, &initial_error);
	...
	for(i = 1; i < max_epochs; i++)
	{
	    ...
	    fann_train_outputs_epoch(ann, data, &error);
	    
	    //printk("Epoch %6d. Current error: %.6f. Bit fail %d.\n", i, error, ann->num_bit_fail); 
		...
		(target_improvement < 0 &&
			(error_improvement < target_improvement || error_improvement > backslide_improvement)))
		{
			/*printk("error_improvement=%f, target_improvement=%f, backslide_improvement=%f, stagnation=%d\n", error_improvement, target_improvement, backslide_improvement, stagnation); */
...
void fann_train_outputs_epoch(struct fann *ann, struct fann_train_data *data, float* ret)
{
	unsigned int i;
...
int fann_reallocate_connections(struct fann *ann, unsigned int total_connections)
{
...
#ifdef CASCADE_DEBUG
	printk("realloc from %d to %d\n", ann->total_connections_allocated, total_connections);
#endif
	ann->connections =
	    (struct fann_neuron **) krealloc(ann->connections, total_connections * sizeof(struct fann_neuron *), GFP_KERNEL);
	...
	ann->weights = (fann_type *) krealloc(ann->weights, total_connections * sizeof(fann_type), GFP_KERNEL);
	if(ann->weights == NULL)
	{
		fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
		return -1;
	...
	ann->train_slopes =
	    (fann_type *) krealloc(ann->train_slopes, total_connections * sizeof(fann_type), GFP_KERNEL);
	...
	ann->prev_steps = (fann_type *) krealloc(ann->prev_steps, total_connections * sizeof(fann_type), GFP_KERNEL);
	...
	ann->prev_train_slopes =
	    (fann_type *) krealloc(ann->prev_train_slopes, total_connections * sizeof(fann_type), GFP_KERNEL);
...
int fann_reallocate_neurons(struct fann *ann, unsigned int total_neurons)
{
	...
	neurons =
	    (struct fann_neuron *) krealloc(ann->first_layer->first_neuron, total_neurons * sizeof(struct fann_neuron), GFP_KERNEL);
	...
	ann->train_errors = (fann_type *) krealloc(ann->train_errors, total_neurons * sizeof(fann_type), GFP_KERNEL);
...
int fann_initialize_candidates(struct fann *ann)
{
	...
	mpow(0.7f * num_hidden_neurons, 1.0f / ann->num_input, &tmp);
	scale_factor = (float) (2.0 * tmp);
...
int fann_train_candidates(struct fann *ann, struct fann_train_data *data)
{
	...
	if(ann->cascade_candidate_scores == NULL)
	{
		ann->cascade_candidate_scores =
		    (fann_type *) kmalloc(fann_get_cascade_num_candidates(ann) * sizeof(fann_type), GFP_KERNEL);
		if(ann->cascade_candidate_scores == NULL)
		{
			fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
			return 0;
	...
	for(i = 0; i < max_epochs; i++)
	{
	    ...
	    fann_train_candidates_epoch(ann, data, &best_cand_score);

		if(best_cand_score / ann->MSE_value > ann->cascade_candidate_limit)
		{
#ifdef CASCADE_DEBUG
			printk("above candidate limit %f/%f > %f", best_cand_score, ann->MSE_value,
		...
		{
#ifdef CASCADE_DEBUG_FULL
			printk("Best candidate score %f, real score: %f\n", ann->MSE_value - best_cand_score,
				   best_cand_score);
			/* printk("best_cand_score=%f, target_cand_score=%f, backslide_cand_score=%f, stagnation=%d\n", best_cand_score, target_cand_score, backslide_cand_score, stagnation); */
#endif
		...
		{
#ifdef CASCADE_DEBUG
			printk("Stagnation with %d epochs, best candidate score %f, real score: %f\n", i + 1,
...
#ifdef CASCADE_DEBUG
	printk("Max epochs %d reached, best candidate score %f, real score: %f\n", max_epochs,
		   ann->MSE_value - best_cand_score, best_cand_score);
#endif
	return max_epochs;
}
...
void fann_update_candidate_slopes(struct fann *ann)
{
	...
	for(cand_it = first_cand; cand_it < last_cand; cand_it++)
	{
		...
		fann_activation(ann, cand_it->activation_function, cand_it->activation_steepness, cand_sum, &activation);
		/* printk("%f = sigmoid(%f);\n", activation, cand_sum); */
		...
		fann_activation_derived(cand_it->activation_function, cand_it->activation_steepness, activation, cand_sum, &derived);
...
void fann_train_candidates_epoch(struct fann *ann, struct fann_train_data *data, fann_type* ret)
{
	unsigned int i, j;
	...
	for(i = 1; i < num_cand; i++)
	{
		 ...
		 * printk("candidate[%d] = activation: %s, steepness: %f, score: %f\n", 
...
#ifdef CASCADE_DEBUG
	printk("Best candidate[%d]: with score %f, real score: %f\n", best_candidate,
...
struct fann_layer *fann_add_layer(struct fann *ann, struct fann_layer *layer)
{
	...
	struct fann_layer *layers =
		(struct fann_layer *) krealloc(ann->first_layer, num_layers * sizeof(struct fann_layer), GFP_KERNEL);
	if(layers == NULL)
	{
		fann_error((struct fann_error *) ann, FANN_E_CANT_ALLOCATE_MEM);
		return NULL;
...
void fann_add_candidate_neuron(struct fann *ann, struct fann_layer *layer)
{
	...
	{
#ifdef CASCADE_DEBUG_FULL
		printk("move neuron pointers in layer %d, first(%d -> %d), last(%d -> %d)\n",
...
#ifdef CASCADE_DEBUG_FULL
	printk("num_connections_in=%d, num_connections_out=%d\n", num_connections_in,
		   num_connections_out);
#endif
	...
	for(neuron_it = (ann->last_layer - 1)->last_neuron - 1; neuron_it != neuron_place; neuron_it--)
	{
		...
		*neuron_it = *(neuron_it - 1);
		...
		/* move the weights */
#ifdef CASCADE_DEBUG_FULL
		printk("move weight[%d ... %d] -> weight[%d ... %d]\n", neuron_it->first_con,
...
#ifdef CASCADE_DEBUG_FULL
	printk("neuron[%d] = weights[%d ... %d] activation: %s, steepness: %f\n",
...
#ifdef CASCADE_DEBUG_FULL
	printk("move cand weights[%d ... %d] -> [%d ... %d]\n", candidate_con,
...
#ifdef CASCADE_DEBUG_FULL
		printk("move weights[%d] -> weights[%d] (%f)\n", i + candidate_con,
														 ...
														 cascade_activation_functions_count)
{
	if(ann->cascade_activation_functions_count != cascade_activation_functions_count)
	{
		...
		ann->cascade_activation_functions = 
			(enum fann_activationfunc_enum *)krealloc(ann->cascade_activation_functions, 
			ann->cascade_activation_functions_count * sizeof(enum fann_activationfunc_enum), GFP_KERNEL);
														   ...
														   cascade_activation_steepnesses_count)
{
	if(ann->cascade_activation_steepnesses_count != cascade_activation_steepnesses_count)
	{
		...
		ann->cascade_activation_steepnesses = 
			(fann_type *)krealloc(ann->cascade_activation_steepnesses, 
			ann->cascade_activation_steepnesses_count * sizeof(fann_type), GFP_KERNEL);
